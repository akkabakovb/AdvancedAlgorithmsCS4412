...
class PriorityQueue:
	...
	def bubbleUp(self, ind):
		if ind <= 0: return
		parent_i = (ind-1) // 2
		if self.priority_q[parent_i][1] > self.priority_q[ind][1]:
			self.priority_q[ind], self.priority_q[parent_i] = self.priority_q[parent_i], self.priority_q[ind]
			return self.bubbleUp(parent_i)
		else: return

	def insert(self, value, key):
		self.priority_q.append((value, key))
		self.bubbleUp(len(self.priority_q)-1)
	
	def percolateDown(self, parent_ind):
		size = len(self.priority_q)
		min = parent_ind
		if 2*parent_ind+1 < size and self.priority_q[2*parent_ind+1][1]<self.priority_q[min][1]:
			min = 2*parent_ind+1
		if 2*parent_ind+2 < size and self.priority_q[2*parent_ind+2][1]<self.priority_q[min][1]:
			min = 2*parent_ind+2
		if min != parent_ind:
			self.priority_q[min], self.priority_q[parent_ind] = self.priority_q[parent_ind], self.priority_q[min]
			self.percolateDown(min)


	def deleteMin(self):
		if not self.priority_q:
			return None
		min_val = self.priority_q[0][0]

		if len(self.priority_q) == 1:
			self.priority_q.pop()
			return min_val
	...
	def getSize(self):
		return len(self.priority_q)

class State:

	def __init__(self, c_matrix, lower_bound, route):
	...
	def reduceRows(self):
		for i in range(len(self.c_matrix)):
			...
			if min_val != 0 and min_val != np.inf:
				...
				self.c_matrix[i] = [x - min_val if x != np.inf else x for x in row]

	def reduceCols(self):
		n = len(self.c_matrix)
		for i in range(n):
			col = [self.c_matrix[row][i] for row in range(n)]
			min_val = min(col)
			if min_val != 0 and min_val != np.inf:
				self.lower_bound += min_val
				for row in range(n):
					if self.c_matrix[row][i] != np.inf:
						self.c_matrix[row][i] -= min_val
	...
	def reduceMatrix(self):
		...
		return self.c_matrix, self.lower_bound
